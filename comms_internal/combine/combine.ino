...
int positionChangeWaitingTime = 4000; // 4s to detect position change
int c = 0, i = 0, count = 0;
int right = 0, left = 0, still = 0;
...
void runMPU(const int MPU) {
  ...
  if ((AccZ - prevAccZ) > 0.025) { // Must increase 5 times consecutively, if decrease, reset variable left and right
    right++;
  }
  else if ((AccZ - prevAccZ) < -0.025)
    left++;
  else {
    right = 0;
    left = 0;
  ...
  // If 5 samples all increase/decrease, print position n exit while loop
  if (left >= 1) {
    L = true;
    right = 0;
    left = 0;
